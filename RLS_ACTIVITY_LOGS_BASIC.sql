...
DROP POLICY IF EXISTS "Authenticated users can delete vdcr activity logs" ON public.vdcr_activity_logs;
...
-- Policy: Authenticated users can delete VDCR activity logs (needed for cascade deletion when deleting VDCR records)
CREATE POLICY "Authenticated users can delete vdcr activity logs"
ON public.vdcr_activity_logs FOR DELETE
TO authenticated
USING (
  -- Allow deletion if user has access to the related VDCR record's project
  EXISTS (
    SELECT 1 FROM public.vdcr_records vr
    WHERE vr.id = vdcr_activity_logs.vdcr_id
    AND (
      -- User is assigned to the project
      public.is_assigned_to_project(vr.project_id)
      -- Or user is firm admin for the project's firm
      OR EXISTS (
        SELECT 1 FROM public.projects p
        JOIN public.users u ON u.id = auth.uid()
        WHERE p.id = vr.project_id
        AND p.firm_id = u.firm_id
        AND u.role = 'firm_admin'
  ...
  -- Also allow if vdcr_id is null (standalone logs)
  OR vdcr_id IS NULL
);
