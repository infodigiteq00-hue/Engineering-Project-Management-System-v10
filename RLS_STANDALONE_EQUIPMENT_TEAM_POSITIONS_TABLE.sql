...
-- Step 2: Drop existing policies first (they depend on the function), then drop the function
-- IMPORTANT: Drop policies BEFORE dropping the function to avoid dependency errors
...
-- Now safe to drop the function after all dependent policies are dropped
DROP FUNCTION IF EXISTS public.is_user_assigned_to_standalone_equipment_for_insert(uuid);

-- NOTE: is_user_assigned_to_standalone_equipment() function is defined in RLS_STANDALONE_EQUIPMENT_TABLE.sql
-- and is used by policies on both standalone_equipment and standalone_equipment_team_positions tables.
-- We don't drop it here to avoid dependency errors. It's already created/updated in the standalone equipment file.
...
-- Policy 3: Users can view ALL team positions for standalone equipment they're assigned to or created
-- FIXED: Now shows all team members for equipment user is assigned to (not just their own record)
-- Similar to how project equipment works - if assigned to equipment, can see all team members
CREATE POLICY "Users can view assigned standalone equipment team positions"
ON public.standalone_equipment_team_positions FOR SELECT
TO authenticated
USING (
  -- User created the equipment - can see all team members
  EXISTS (
    SELECT 1 FROM public.standalone_equipment se
    WHERE se.id = standalone_equipment_team_positions.equipment_id
  ...
  OR
  -- User is assigned to the equipment - can see all team members (using SECURITY DEFINER function)
  public.is_user_assigned_to_standalone_equipment(standalone_equipment_team_positions.equipment_id)
);
